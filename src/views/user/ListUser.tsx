...
const ListUser = () => {
  ...
  const fetchUser = async () => {
    const res = await axiosServices.get(`${process.env.REACT_APP_API_URL}/v1/client/users?page=1`, {
      headers: {
  ...
  const handleDelete = (id: number) => {
    console.log(id);
    try {
      axiosServices.delete(`${process.env.REACT_APP_API_URL}/v1/operator/users/${id}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
